...
void mat3Rotate(float r, mat3 out) {
    float c = cosf(r);
    float s = sinf(r);
    out[0 + 0 * 3] = c; out[1 + 0 * 3] = -s; out[2 + 0 * 3] = 0.0f;
    out[0 + 1 * 3] = s; out[1 + 1 * 3] = c; out[2 + 1 * 3] = 0.0f;
}

void mat3Scale(vec2 s, mat3 out) {
    out[0 + 0 * 3] = s[0]; out[1 + 0 * 3] = 0.0f; out[2 + 0 * 3] = 0.0f;
    out[0 + 1 * 3] = 0.0f; out[1 + 1 * 3] = s[1]; out[2 + 1 * 3] = 0.0f;
}
